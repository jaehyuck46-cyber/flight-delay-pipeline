...
```
    ...
    ⏰ GitHub Actions cron(*/30) 이 위 흐름 전체를 자동 실행하고,
      갱신된 DB를 저장소에 커밋한다.
      (*목표 주기 30분. 단 플랫폼 혼잡 시 실제 간격은 더 벌어질 수 있음
        → 정시성 필요 시 AWS EventBridge+Lambda 이관 예정*)
```
...
- [ ] **예보→관측(ASOS) 전환** — 과거 데이터 백필로 flights×weather 매칭률 개선
